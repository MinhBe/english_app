...
const publicKey =
  ...
  firstEnvEndingWith('_SUPABASE_ANON_KEY') ||
  firstEnvEndingWith('_SUPABASE_PUBLISHABLE_KEY');

if (!databaseUrl || !directUrl) {
  throw new Error('No Postgres connection was found in Vercel environment variables.');
}
if (!supabaseUrl || !adminKey || !publicKey) {
  throw new Error('Supabase Auth environment variables are incomplete.');
}
...
console.log('Bootstrap: synchronizing Prisma schema.');
...
for (const spec of testUsers) {
  ...
  const { data: login, error: loginError } = await probe.auth.signInWithPassword({
    email: spec.email,
    password: spec.password,
  });
...
console.log(`Bootstrap complete: ${profileCount} profile(s); test logins verified.`);
